require "pry"
  ...
  def company_jobs(comp_id)
    ...
    jobs_in_company.each do |job|
      ...
      puts "#{job.description[0..250].gsub!(/(<[^>]*>)|\n|\t/s) {" "}}..."
      puts
      puts "Apply: #{job.how_to_apply.gsub!(/(<[^>]*>)|\n|\t/s) {" "}}"
  ...
  def delete_stupid_words(string)
    modified_string = string.delete("<p><strong>")
    modified_string = string.delete("!</strong></p>")
